...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Filter posts for client review if user is client
    const q = userRole === 'client'
      ? query(
          collection(db, 'posts'),
          where('phase', 'in', ['idea_2', 'copy', 'design', 'client_review', 'approved', 'published'])
        )
  ...
  useEffect(() => {
    ...
    setHistory([
      { createdAt: new Date(), authorName: 'Admin', copyCaption: 'Test version' }
    ]);
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans">
      ...
      <aside className="w-64 bg-white border-r border-gray-100 p-6 flex flex-col shrink-0 hidden lg:flex h-screen sticky top-0 overflow-hidden justify-between">
        <div className="flex flex-col overflow-hidden flex-1">
          ...
          {/* Project Selector with Create Button */}
          <div className="mb-6 bg-slate-50 border border-slate-100/80 p-3 rounded-2xl shrink-0">
            <label className="block text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-2">Proyecto / Cliente</label>
            <div className="flex gap-1.5 items-center">
              <select
                value={activeProjectId}
                onChange={(e) => setActiveProjectId(e.target.value)}
                className="w-full bg-white border border-gray-200 rounded-xl py-2 px-2.5 text-xs font-semibold text-gray-700 focus:outline-none focus:ring-2 focus:ring-app-accent/10 cursor-pointer"
              >
              ...
              {userRole !== 'client' && (
                <button 
                  ...
                  className="p-2 bg-app-accent hover:bg-app-accent-hover text-white rounded-xl transition-all shadow-sm shrink-0"
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-20 bg-white border-b border-gray-100 px-6 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-4">
            {/* Topbar Client/Project Selector */}
            <div className="flex items-center gap-2 bg-gray-100/70 border border-gray-200/50 px-3 py-1.5 rounded-xl">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider hidden sm:inline">Cliente:</span>
              <select
                value={activeProjectId}
                onChange={(e) => setActiveProjectId(e.target.value)}
