...
export const PLATFORM_META: Record<SocialPlatform, { label: string; color: string; Icon: React.FC<IconProps> }> = {
  instagram: { label: 'Instagram', color: '#E1306C', Icon: InstagramIcon },
  linkedin: { label: 'LinkedIn', color: '#0A66C2', Icon: LinkedInIcon },
  tiktok: { label: 'TikTok', color: '#18181b', Icon: TikTokIcon },
};
...
export const PlatformBadge: React.FC<PlatformBadgeProps> = ({ platform, size = 16, className, showLabel = false }) => {
  const meta = PLATFORM_META[platform];
  const { Icon } = meta;
  return (
    <span className={`inline-flex items-center gap-1.5 ${className ?? ''}`}>
      <Icon size={size} style={{ color: meta.color }} />
      {showLabel && <span className="capitalize">{meta.label}</span>}
    </span>
  );
};
